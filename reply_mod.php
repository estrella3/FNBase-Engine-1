...
if($a == 'edit'){
    $uid = $_SESSION['userid'];
    $sql = "SELECT * FROM `_reply` WHERE `num` LIKE '$id' AND `id` LIKE '$uid'";
    $result = mysqli_query($conn, $sql);
    while($row = mysqli_fetch_array($result)){
    $c = $row['content'];
    }
    echo '<form action="reply_mod.php?a=save&n='.$id.'" method="post">
...
if($a == 'save'){
    $c = $_POST['ec'];
    $sql = "UPDATE `_reply` set content = '$c', edited = 1 WHERE `num` LIKE '$id'";
    $result = mysqli_query($conn, $sql);
    if($result === FALSE){
    echo "데이터베이스 접속 오류";
    }
...
if($a == 'delete'){
    if($_GET['c'] == 'ok'){
        $sql = "SELECT * FROM `_reply` WHERE `num` LIKE '$id'";
        $result = mysqli_query($conn, $sql);
        while($row = mysqli_fetch_array($result)){
        $uid = $row['id'];
        $b = $row['board'];
        ...
        if($uid === $_SESSION['userid']){
            ...
            echo '<div class="container"><form action="reply_mod.php?a=delete&c=ok&n='.$id.'" method="post">
...
if($a == 'push'){
    ...
    $sql = "UPDATE `_reply` set stat = $stat WHERE `num` LIKE '$id'";
    $result = mysqli_query($conn, $sql);
    if($result === FALSE){
        echo "데이터베이스 접속 오류";
    }else{
...
if($a == 'blame'){
    ...
    $sql = "UPDATE `_reply` set blame = $stat WHERE `num` LIKE '$id'";
    $result = mysqli_query($conn, $sql);
    if($result === FALSE){
        echo "데이터베이스 접속 오류";
    }else{
